...
function MainFeedSection() {
    ...
    if(!loading && !error && feeds.length === 0) return (
        <div className='col-span-7 h-screen flex flex-col items-center justify-center'>
    ...
    return (
        <div className='flex flex-col gap-y-8 col-span-7 overflow-y-scroll h-screen py-10 items-center'>
            {feeds.map((feed: any) => (
                <SingleFeed key={feed._id} feed={feed}/>
            ))}
        </div>
    );
}
